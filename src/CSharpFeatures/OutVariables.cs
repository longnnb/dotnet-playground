...
public class OutVariables
{
    public void TryParseInt()
    {
        ...
        if (int.TryParse(input, out var result)) WriteLine(result);
    ...
    {
        DateTime dt; // struct
        if (DateTime.TryParse("01/01/2017", out dt)) WriteLine($"Old-fashioned parse: {dt}");

        // variable declaration is an expression, not a statement
        if (DateTime.TryParse("02/02/2016", out /*DateTime*/ var dt2)) WriteLine($"New parse: {dt2}");
