...
- `CollectionTypes` — the BCL collection types and the pitfalls that bite: `List<T>` capacity growth and the remove-while-iterating bug, `Dictionary<TKey,TValue>` lookup APIs / enumeration order / `CollectionsMarshal` ref access / `GetAlternateLookup`, `HashSet`/`SortedSet` algebra, the ordered/specialized collections (`Queue`/`Stack`/`LinkedList`/`PriorityQueue`/`SortedDictionary` vs `SortedList`), custom `IEqualityComparer` and the mutated-hash-key trap, `ReadOnlyCollection` (a view, not a copy) vs immutable vs frozen, `System.Collections.Concurrent` (`GetOrAdd` running its factory more than once, `BlockingCollection`), collection expressions / `params` spans / `[CollectionBuilder]`, and writing a `yield` iterator / custom `ICollection<T>`. Named `CollectionTypes` rather than `Collections` for the same reason as `CancellationTokenWpf`: a root namespace `Collections` shadows `System.Collections`
